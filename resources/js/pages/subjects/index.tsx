...
export default function SubjectsIndex({ subjects, filters, flash }: Props) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            router.get('/subjects', { search }, { preserveState: true, preserveScroll: true, replace: true });
        }, 300);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (modal === 'create') {
            form.post('/subjects', { onSuccess: () => closeModal() });
        } else if (modal === 'edit' && editTarget) {
            form.put(`/subjects/${editTarget.id}`, { onSuccess: () => closeModal() });
        }
    };
    ...
    const TypeBadge = ({ type }: { type: 'mandatory' | 'optional' }) => (
        <span
            className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${
                type === 'mandatory'
                    ? 'bg-blue-50 text-blue-700 ring-blue-700/10 dark:bg-blue-900/20 dark:text-blue-400 dark:ring-blue-400/20'
                    : 'bg-green-50 text-green-700 ring-green-600/10 dark:bg-green-900/20 dark:text-green-400 dark:ring-green-400/20'
            }`}
        >
                    ...
                    {currentSubjects.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            ...
                            {search ? (
                                ...
                                <>
                                    <p className="text-sm font-medium text-gray-900 dark:text-white">No subjects yet</p>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by creating your first subject.</p>
                            ...
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Subject Name</th>
                                        <th className="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Code</th>
                                        <th className="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Type</th>
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                    {currentSubjects.map((subject, index) => (
                                        <tr key={subject.id} className="group transition hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                            <td className="px-6 py-4 text-sm text-gray-400 dark:text-gray-500">{(subjects.from || 1) + index}</td>
                                            <td className="px-6 py-4">
                                                <span className="text-sm font-medium text-gray-900 dark:text-white">
                                                    {subject.name}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-sm font-mono text-gray-600 dark:text-gray-300">{subject.code}</td>
                                            <td className="px-6 py-4">
                ...
                {subjects.total > 0 && (
                    <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            Showing <span className="font-medium text-gray-900 dark:text-white">{subjects.from || 0}</span> to{' '}
                            <span className="font-medium text-gray-900 dark:text-white">{subjects.to || 0}</span> of{' '}
                            <span className="font-medium text-gray-900 dark:text-white">{subjects.total}</span> subjects
                        </p>
                    ...
                    <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900 ring-1 ring-gray-900/10 dark:ring-white/10 transition-all duration-300">
                        <div className="mb-5 flex items-center justify-between">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {modal === 'create' ? 'Add Subject' : 'Edit Subject'}
                            </h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="flex justify-end gap-3 pt-4">
                                ...
                                >
                                    {form.processing ? 'Saving…' : modal === 'create' ? 'Add Subject' : 'Save Changes'}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 transition-all duration-300">
                    <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900 ring-1 ring-gray-900/10 dark:ring-white/10">
                        ...
                        <h3 className="text-base font-semibold text-gray-900 dark:text-white">Delete Subject</h3>
                        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            Are you sure you want to delete{' '}
                            <strong>{deleteTarget.name} ({deleteTarget.code})</strong>? This action cannot be undone.
                        </p>
